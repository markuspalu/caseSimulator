...
const Opening = ({ fakeList, isOpen, setIsOpen, selectItem, openCase, listUpdated, setListUpdated }) => {
  
  const rollCase = () => {
    console.log("rollCase");
    openCase();
    setIsOpen(true);
    setTimeout(() => {
      setIsOpen(false);
      console.log("finished rollCase")
    }, 8000)
  }
  ...
  return (
    ...
    <div className='scroller-container'>
      <div className={`scroller ${isOpen ? 'open' : ''}`}>
        {listUpdated && fakeList.map((item, index) => {
...
function App() {
  ...
  const selectItem = (color) => {
    ...
    let chanceItem = Math.floor(Math.random() * 3);
    const selectedItem = items.itemSelection[color].items[chanceItem]; // we get selected item
    console.log("selectedItem: " + selectedItem);
    fillList() // we fill the list

    setTimeout(() => {
      setOpenedItem(selectedItem); // You opened (item)
      setOpenedColor(items.itemSelection[color]); // You opened (color) 
      setItemList(prevList => { // Update inventory
  ...
  return (
    ...
    <div className="flex flex-col items-center">
      <Opening fakeList={fakeList} isOpen={isOpen} setIsOpen={setIsOpen} selectItem={selectItem} openCase={openCase} listUpdated={listUpdated} setListUpdated={setListUpdated} />
